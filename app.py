...
def obtener_bytes_imagen(logo_src):
    """Convierte cualquier fuente de imagen (URL, Base64, UploadedFile, Archivo) en BytesIO."""
    if not logo_src:
        return None

    # 1. Objeto de Streamlit (UploadedFile) o Stream con getvalue()/read()
    if hasattr(logo_src, "getvalue"):
        return io.BytesIO(logo_src.getvalue())
    if hasattr(logo_src, "read"):
        return io.BytesIO(logo_src.read())

    # 2. Si ya son Bytes directos
    if isinstance(logo_src, bytes):
        return io.BytesIO(logo_src)

    logo_str = str(logo_src).strip()

    # 3. Formato Data URI Base64 ("data:image/png;base64,...")
    if logo_str.startswith("data:image"):
        try:
            base64_data = logo_str.split(",")[1]
            return io.BytesIO(base64.b64decode(base64_data))
        except Exception:
            pass

    # 4. Cadena Base64 pura
    if len(logo_str) > 100 and not logo_str.startswith("http") and not os.path.exists(logo_str):
        try:
            return io.BytesIO(base64.b64decode(logo_str))
        except Exception:
            pass

    # 5. URL HTTP o HTTPS (Con bypass de verificación SSL y User-Agent)
    if logo_str.startswith("http://") or logo_str.startswith("https://"):
        try:
            headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
            resp = requests.get(logo_str, headers=headers, timeout=5, verify=False)
            if resp.status_code == 200:
                return io.BytesIO(resp.content)
        except Exception:
            pass

    # 6. Ruta de archivo en el sistema local
...
def generar_guia_pdf(
    ...
    d_emp = datos_emp if isinstance(datos_emp, dict) else {}

    nombre_empresa = (d_emp.get("razon_social") or d_emp.get("nombre_empresa") or d_emp.get("nombre") or cfg.get("nombre_empresa") or "MI EMPRESA SPA")
    rut_empresa = (d_emp.get("rut_empresa") or d_emp.get("rut") or cfg.get("rut_empresa") or "Sin RUT")
    giro_empresa = (d_emp.get("giro") or cfg.get("giro") or "VENTA AL POR MAYOR EN COMERCIOS")
    direccion_empresa = (d_emp.get("direccion") or cfg.get("direccion") or "Sin Dirección")
    comuna_empresa = d_emp.get("comuna", "")
    if comuna_empresa:
        direccion_empresa += f", {comuna_empresa}"

    # Búsqueda exhaustiva del Logo
    logo_src = (
        logo_override or
        d_emp.get("logo_url") or 
        d_emp.get("logo") or 
        d_emp.get("url_logo") or 
        d_emp.get("imagen") or 
        d_emp.get("avatar_url") or
        cfg.get("logo_url") or 
        cfg.get("logo") or 
        cfg.get("url_logo") or
        st.session_state.get("logo_url") or 
        st.session_state.get("logo") or
        st.session_state.get("logo_empresa")
    )
    ...
    c_nombre = datos_receptor.get("nombre") or cliente_nombre or "CLIENTE CONTADO"
    c_rut = datos_receptor.get("rut") or cliente_rut or "66666666-6"
    c_giro = datos_receptor.get("giro") or "Particular / Consumidor Final"
    c_dir = datos_receptor.get("direccion") or "N/A"
    c_comuna = datos_receptor.get("comuna") or "Santiago"
    ...
    if logo_src:
        try:
            img_bytes = obtener_bytes_imagen(logo_src)
            if img_bytes:
                img = Image.open(img_bytes)

                # Convertir transparencias PNG/WEBP (RGBA) a fondo blanco RGB
                ...
                # Guardar en un archivo temporal seguro para FPDF
                ...
                st.warning(f"⚠️ Se encontró la clave del logo pero no se pudo leer la imagen.")
        except Exception as e:
            st.warning(f"⚠️ Error al renderizar el logo en el PDF: {e}")
            has_logo = False
        finally:
    ...
    if has_logo:
        text_x_offset = 40  # Desplaza los datos de la empresa para dar espacio al logo
    ...
    pdf.cell(75, 6, f"R.U.T. : {rut_empresa}", ln=True, align="C")

    pdf.set_font("Arial", "B", 11)
    pdf.set_x(125)
    pdf.cell(75, 6, str(tipo_documento).upper(), ln=True, align="C")

    pdf.set_font("Arial", "B", 12)
    pdf.set_x(125)
    pdf.cell(75, 7, f"N° {folio}", ln=True, align="C")
    ...
    pdf.cell(184, 5, "DATOS RECEPTOR", ln=True)

    pdf.set_text_color(40, 40, 40)
    pdf.set_font("Arial", "", 8.5)
    pdf.set_x(13)
    pdf.cell(184, 4.5, f"Cliente: {c_nombre}   |   RUT: {c_rut}", ln=True)
    pdf.set_x(13)
    pdf.cell(184, 4.5, f"Giro: {c_giro}   |   Dirección: {c_dir}, {c_comuna}", ln=True)
...
# ----------------- SECCIÓN DASHBOARD EJECUTIVO -----------------
        ...
        if res_v.data:
            ...
            if not df_v.empty:
                ...
                if not df_v.empty:
                    ...
                    if col_fecha_v:
                        df_v['Fecha_Parsed'] = pd.to_datetime(df_v[col_fecha_v], errors='coerce')
                    ...
                    if not df_ventas_filtrado.empty:
                        col_monto = next((c for c in ['monto', 'total', 'monto_total'] if c in df_ventas_filtrado.columns), None)
                        if col_monto:
                            total_ventas_periodo = float(pd.to_numeric(df_ventas_filtrado[col_monto], errors='coerce').fillna(0).sum())

                        if 'iva' in df_ventas_filtrado.columns:
                            total_debito_fiscal = float(pd.to_numeric(df_ventas_filtrado['iva'], errors='coerce').fillna(0).sum())
                        elif 'monto_iva' in df_ventas_filtrado.columns:
                            total_debito_fiscal = float(pd.to_numeric(df_ventas_filtrado['monto_iva'], errors='coerce').fillna(0).sum())
                        else:
        ...
        if res_c.data:
            ...
            if not df_c.empty:
                ...
                if not df_c.empty:
                    ...
                    if col_fecha_c:
                        df_c['Fecha_Parsed'] = pd.to_datetime(df_c[col_fecha_c], errors='coerce')
                    ...
                    if not df_compras_filtrado.empty:
                        col_monto_c = next((c for c in ['costo_total', 'CostoTotal', 'monto_total', 'monto', 'total'] if c in df_compras_filtrado.columns), None)
                        col_iva_c = next((c for c in ['monto_iva', 'iva', 'IVA'] if c in df_compras_filtrado.columns), None)
                        ...
                        if col_iva_c:
                            ...
                            credito_fiscal_compras = float(compras_totales_periodo * (0.19 / 1.19))
    except Exception as e:
            ...
            if not df_g.empty:
                ...
                if not df_g.empty:
                    ...
                    if not df_g_filtrado.empty:
                        col_monto_g = next((c for c in ['monto', 'total', 'monto_total'] if c in df_g_filtrado.columns), None)
                        col_iva_g = next((c for c in ['monto_iva', 'iva', 'IVA'] if c in df_g_filtrado.columns), None)
                        
                        if col_monto_g:
                            if col_iva_g:
                                credito_fiscal_gastos = float(pd.to_numeric(df_g_filtrado[col_iva_g], errors='coerce').fillna(0).sum())
                            else:
                                credito_fiscal_gastos = float(pd.to_numeric(df_g_filtrado[col_monto_g], errors='coerce').fillna(0).sum() * (0.19 / 1.19))

                            for _, row in df_g_filtrado.iterrows():
                                monto = float(pd.to_numeric(row.get(col_monto_g, 0), errors='coerce') or 0)
    ...
    if not df_v.empty:
        ...
        if col_monto_v:
            total_ventas_historico = float(pd.to_numeric(df_v[col_monto_v], errors='coerce').fillna(0).sum())
    ...
    if not df_c.empty:
        ...
        if col_monto_c_h:
            total_compras_historico = float(pd.to_numeric(df_c[col_monto_c_h], errors='coerce').fillna(0).sum())

    total_gastos_historico = 0.0
    if not df_g.empty:
        col_monto_g_h = next((c for c in ['monto', 'total', 'monto_total'] if c in df_g.columns), None)
        if col_monto_g_h:
            total_gastos_historico = float(pd.to_numeric(df_g[col_monto_g_h], errors='coerce').fillna(0).sum())
    ...
    with col_b1_5:
        ...
        st.metric(
            ...
            help="Fórmula Real: (Ventas Históricas + Fondo Inyectado) - (Compras Históricas + Gastos Históricos)."
        )
    ...
    with col_g2:
        ...
        if not df_g_filtrado.empty and 'categoria' in df_g_filtrado.columns:
            col_m_g = next((c for c in ['monto', 'total', 'monto_total'] if c in df_g_filtrado.columns), None)
            if col_m_g:
...
# ----------------- SECCIÓN VENTAS / POS RÁPIDO (CONECTADO A LA NUBE Y AISLADO / HÍBRIDO OFFLINE) -----------------
    ...
    if modo_online:
        cant_sinc = sincronizar_ventas_pendientes_pos(supabase)
        if cant_sinc > 0:
    ...
    if not rut_actual:
        ...
        if negocios:
            ...
            st.error("⚠️ No se ha detectado ningún negocio seleccionado en la sesión.")
            st.stop()
    ...
    if "procesando_emision_dte" not in st.session_state: st.session_state.procesando_emision_dte = False
        ...
        if codigo_leido:
            ...
            if df_a_buscar is not None and not df_a_buscar.empty:
                ...
                if not df_match.empty:
                    ...
                    st.warning(f"⚠️ No se encontró producto con código: {codigo_leido}")
            else:
                st.error("⚠️ La base de datos local no está cargada.")
        st.session_state.input_scanner = ""

    # --- 6. CABECERA, DOCUMENTO Y MODO DE EMISIÓN ---
            ...
            with col_g2:
                ...
                if st.button("📥 Cargar Guía", use_container_width=True):
                    if folio_guia_a_facturar and modo_online:
                        ...
                        st.warning("⚠️ Búsqueda de guías requiere conexión activa.")
                    else:
                        st.warning("⚠️ Ingresa un folio válido.")
    st.markdown("---")

    # --- 8. SELECCIÓN DE CLIENTE Y DATOS TRIBUTARIOS REQUERIDOS ---
    ...
    lista_clientes.insert(0, "-- Selecciona un cliente (Opcional / Requerido para Crédito y Factura) --")
    ...
    if (tipo_documento == "Factura Electrónica" or modo_operacion == "Oficial (SII)") and (not cliente_elegido or cliente_elegido.startswith("--")):
        with st.expander("🏛️ Datos Tributarios del Receptor (Obligatorios para Facturas SII)", expanded=(tipo_documento == "Factura Electrónica")):
            ...
            with col_tr1:
                giro_receptor = st.text_input("Giro Comercial:", value=giro_receptor, placeholder="Ej: Comercio de Abarrotes")
            with col_tr2:
                dir_receptor = st.text_input("Dirección:", value=dir_receptor, placeholder="Ej: Av. San Martín 456")
            with col_tr3:
                comuna_receptor = st.text_input("Comuna:", value=comuna_receptor, placeholder="Ej: San Felipe")
    ...
    if st.session_state.ultimo_recibo is not None:
        ...
        datos_emisor = obtener_datos_emisor(supabase, rut_actual) if modo_online else {}
        
        datos_receptor = {
            "nombre": cliente_nombre if cliente_nombre else "CLIENTE CONTADO",
            "rut": cliente_rut if cliente_rut else "66666666-6",
            "giro": giro_receptor,
            "direccion": dir_receptor,
            "comuna": comuna_receptor
        }

        tot_general = sum(float(i.get('Subtotal', 0)) for i in items_a_mostrar)
        tot_neto = round(tot_general / 1.19)
        tot_iva = tot_general - tot_neto
        ...
        with col_pdf:
            ...
            if st.session_state.get("url_pdf_oficial"):
                ...
                try:
                    ...
                    st.download_button(
                        label=f"📄 Descargar {tipo_documento} (PDF Interno)",
                        data=pdf_bytes,
    ...
    # =========================================================================
    # --- VISTA 2: PANTALLA DE PAGO Y CONFIRMACIÓN CON DTE ---
    # =========================================================================
    elif st.session_state.estado_pago:
        st.markdown("### 💳 2. Formas de Pago y Emisión Tributaria")
        if len(st.session_state.carrito_ventas) > 0:
            ...
            st.info(f"💰 **Total a Pagar: ${total_venta:,.2f}** | Modo: **{modo_operacion}**")
            ...
            with col_p1:
                if st.button("⬅️ Volver al Carrito", use_container_width=True, disabled=st.session_state.procesando_emision_dte):
                    st.session_state.estado_pago = False
                    st.rerun()

            with col_p2:
                btn_confirmar = st.button(
                    "✅ Confirmar Pago y Emitir" if not st.session_state.procesando_emision_dte else "⏳ Procesando DTE...",
                    use_container_width=True, 
                    type="primary",
                    disabled=st.session_state.procesando_emision_dte
                )
                ...
                if btn_confirmar:
                    if forma_pago == "Efectivo" and efectivo_recibido < total_venta:
                        ...
                        st.session_state.procesando_emision_dte = True
                        es_offline_para_cobro = not hay_conexion_activa()
                        fecha_hora_actual = datetime.now()
                        
                        # --- 1. PROCESAR DTE OFICIAL (SI APLICA) ---
                        resultado_dte = None
                        numero_folio_actual = None
                        pdf_url_dte = None
                        xml_url_dte = None
                            ...
                            if not es_offline_para_cobro:
                                try:
                                    ...
                                    if res_f.data and len(res_f.data) > 0:
                                        num_f = int(res_f.data[0]["ultimo_folio_usado"]) + 1
                                        supabase.table("folios_empresa").update({"ultimo_folio_usado": num_f}).eq("rut_empresa", str(rut_actual)).eq("tipo_documento", tipo_documento).eq("modo", modo_str).execute()
                                        numero_folio_actual = str(num_f)
                                    else:
                                    ...
                                    numero_folio_actual = datetime.now().strftime("%H%M%S")
                            else:
                                numero_folio_actual = datetime.now().strftime("%H%M%S")
                        ...
                        # Eliminar guía previa si corresponde
                        ...
                        for item in st.session_state.carrito_ventas:
                            ...
                            # Descontar stock atómicamente si está online
                            if not es_offline_para_cobro and not item.get("es_guia_previa", False):
                                try:
                                    codigo_vendido = str(item["Código"])
                                    cantidad_vendida = float(item["Cantidad"])

                                    res_receta = supabase.table("recetas").select("*").eq("rut_empresa", str(rut_actual)).eq("codigo_producto_final", codigo_vendido).execute()
                                    if res_receta.data:
                                        for comp in res_receta.data:
                                            supabase.rpc('actualizar_stock_atomico', {
                                                'p_rut_empresa': str(rut_actual),
                                                'p_codigo': str(comp["codigo_ingrediente"]),
                                                'p_bodega': str(bodega_actual),
                                                'p_cantidad': float(comp["cantidad_usada"]) * cantidad_vendida,
                                                'p_operacion': 'VENTA'
                                            }).execute()
                                    else:
                                        supabase.rpc('actualizar_stock_atomico', {
                            ...
                            neto_calc = monto_bruto / (1.0 + tasa_iva_item)
                            iva_calc = monto_bruto - neto_calc

                            total_neto_ticket += neto_calc
                            total_iva_ticket += iva_calc
                        ...
                        if forma_pago in ["Crédito", "Consignación"] and not es_offline_para_cobro:
                            try:
                                supabase.table("cuentas_por_cobrar").insert({
                                    ...
                                    "cliente": cliente_nombre.strip() if cliente_nombre else "Cliente General",
                                    "rut_cliente": cliente_rut.strip() if cliente_rut else "Sin RUT",
                                    "monto_total": float(total_venta),
                                    "saldo_pendiente": float(total_venta),
                                    "fecha_emision": fecha_emision_venta.strftime("%Y-%m-%d"),
                                    "fecha_vencimiento": (fecha_hora_actual + timedelta(days=dias_credito)).strftime("%Y-%m-%d"),
                                    "estado": "Pendiente"
                                }).execute()
                            except Exception as e:
                                print(f"Error registrando CxC: {e}")

                        # Armar Ticket Térmico
                        st.session_state.items_recibo_actual = st.session_state.carrito_ventas.copy()
                        info_pago = f"RECIBIDO: ${efectivo_recibido:,.2f}\nVUELTO: ${cambio:,.2f}" if forma_pago == 'Efectivo' else f"CONDICIÓN: {forma_pago.upper()} ({dias_credito} DÍAS)"
                        
                        texto_recibo = f"""
========================================
       {cfg_actual.get('nombre_empresa', 'MI EMPRESA')}
       RUT: {cfg_actual.get('rut_empresa', rut_actual)}
========================================
DOCUMENTO: {tipo_documento.upper()} [{modo_operacion.upper()}]
FOLIO N°: {numero_folio_actual}
FECHA: {fecha_emision_venta.strftime('%d/%m/%Y')}
----------------------------------------
CLIENTE: {cliente_nombre if cliente_nombre else 'CLIENTE CONTADO'}
RUT: {cliente_rut if cliente_rut else '66666666-6'}
----------------------------------------
DETALLE:
{lineas_productos}----------------------------------------
SUBTOTAL NETO: ${total_neto_ticket:,.2f}
IVA (19%): ${total_iva_ticket:,.2f}
----------------------------------------
